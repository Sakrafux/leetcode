...
We simply need a value map containing the mappings from character to integer value. Iterating through the string, we only need to do a look-ahead for the next character's value in order to check whether we have a substraction notation or not, and then add the value to the result.
